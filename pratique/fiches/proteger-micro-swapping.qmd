...
lfs_2023_pr_swap <- lfs_2023 |>
  # select(all_of(c(risk_variables, hierarchy, unlist(similar), sens_vars, hhid_vars))) |>
...
Par exemple, l'individu 3 a été échangé:

```{r}
lfs_2023 |> slice(c(3,lfs_swapped|>slice(3)|>pull(ID_swapped)))
```
...
```{r}
# lfs_pert <- bind_cols(
#   lfs_2023_pr_swap |> select(REG, DEP, ARR),
#   lfs_swapped |> select(-REG, -DEP, -ARR) |> arrange(ID_swapped) 
# ) 
```
 -->
